    ...
    return (
        <div className="w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl overflow-x-auto no-scrollbar px-4">
            ...
            <div className="flex gap-4 pb-4">
                {/* Add a story */}
                <div onClick={()=> setShowModal(true)} className="flex-shrink-0 rounded-lg shadow-sm w-32 h-40 cursor-pointer hover:shadow-lg transition-all duration-300 border-2 border-dashed border-indigo-300 bg-gradient-to-b from-indigo-50 to-white">
                                ...
                                <div className="absolute inset-0 z-1 rounded-lg bg-black overflow-hidden">
                                    {
                                        story.media_type === 'image' ?
                                            <img src={story.media_url} alt="" className="h-full w-full object-cover hover:scale-110 transition duration-300 opacity-70 hover:opacity-80" /> : <video src={story.media_url} className="h-full w-full object-cover hover:scale-110 transition duration-300 opacity-70 hover:opacity-80" />}
                                </div>
                            )}
                    </div>
                ))}
            ...
            {/* View Story Modal */}
            {viewStory && <StoryViewer viewStory={viewStory} setShowModal={setViewStory} /> }
        </div>
    );
};
